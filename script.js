...
function loadQuestion(index) {
  ...
  if (results[index]) {
    ...
    ["qualityA","semanticA","temporalA","qualityB","semanticB","temporalB"].forEach(id => {
      document.getElementById(id).value = prev[id];
    });
  }
}
...
window.onload = () => {
  ...
  document.getElementById("submitBtn").addEventListener("click", () => {
    ...
    const fields = ["qualityA","semanticA","temporalA","qualityB","semanticB","temporalB"];
    const scores = {};
    for (let f of fields) {
      const val = document.getElementById(f).value;
      if (!val) { alert("Please rate all dimensions for both videos."); return; }
      scores[f] = val;
    }
